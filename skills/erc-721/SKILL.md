...
An ERC-721 contract lives directly in the existing contracts package (Hardhat or Foundry). No new workspace is needed. After deployment, `deployedContracts.ts` auto-generates the ABI and address, so the frontend can interact with the NFT contract using SE-2's scaffold hooks (`useScaffoldReadContract`, `useScaffoldWriteContract`, `useScaffoldContract`).
